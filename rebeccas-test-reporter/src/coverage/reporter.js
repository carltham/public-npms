...
var CoverageReporter = function (rootConfig, helper, logger, emitter) {
  ...
  if (config.watermarks) {
    config.watermarks = helper.merge(
      {},
      istanbulLibReport.getDefaultWatermarks(),
      config.watermarks
    );
  }
  ...
  function checkCoverage(browser, coverageMap) {
    ...
    var eachTrackedFiles = getTrackedFiles(
      coverageMap,
    ...
    eachTrackedFiles.forEach(function (key) {
      var keyThreshold = helper.merge(
        thresholds.each,
        overrideThresholds(key, thresholds.each.overrides)
      );
      check(
        "per-file" + " (" + key + ") ",
        keyThreshold,
        eachResults[key].toJSON()
      );
    });
  ...
  this.onBrowserStart = function (browser) {
    ...
    coverageMaps[browser.id] = istanbulLibCoverage.createCoverageMap(
      startingMap
    );
  };
  ...
  this.onSpecComplete = function (browser, result) {
    console.log("this.onSpecComplete::result = ", result);
    var coverageMap = coverageMaps[browser.id];
  ...
  this.executeReport = async function (reporterConfig, browser) {
    ...
    const toDisk =
      !reporterConfig.type ||
      !reporterConfig.type.match(/^(text|text-summary|in-memory)$/);
